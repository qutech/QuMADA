...
class DacReader:
    ...
    def _script_set_channel(self):
        ...
        """
        return self.ask_raw(f"B{self._slot};C{self._channel};")
    ...
    def _query_address(self, addr: int, count: int = 1, versa_eeprom: bool = False):
        ...
        # Validate address
        addr = int(addr)  # TODO: cast to int not necessary?
...
class DacChannel(InstrumentChannel, DacReader):
    ...
    def _script_ramp(self, start, end, duration, trigger=0):
        ...
        _control_str += f"U{end_val};" if _sign > 0 else f"L{end_val};"
...
class Decadac(VisaInstrument, DacReader):
    ...
    def trigger(self, trigger_setting: str):
        ...
        command = "{*1:B4;C3;D32768;G" + str(trigger_dict[trigger_setting])
    ...
    def _feature_detect(self):
        ...
        try:
            self._EEPROM_available = self._query_address(1107296256) == 21930
        except DACException:
        ...
        with contextlib.suppress(DACException):
