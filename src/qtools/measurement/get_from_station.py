...
_FLAG_FIRST = object()
def flattenDict(d, join = lambda a,b : [a,b], lift=lambda x:x):
    '''
...
def _find_instr_channel(key, station, last_entry = [], **kwargs):
    ...
    f_d = flattenDict(dictionary, join = lambda a,b : [a,b])
    for entry in f_d:
        ...
        if isinstance(entry, list):
            for item in entry:
                ...
                try:
                    ...
                    if _find_instr_channel(
                            dictionary = get_channel(station, entry).__dict__,
                                      key = key, station = station) != []:
                        ...
                        results.append(
                            _find_instr_channel(
                                dictionary = get_channel(station, entry),
                                           key = key, station = station,
...
def find_instr_channels(station, key, submodules = True, **kwargs):
    ...
    if submodules == False:
        return flatten_list(
            _find_instr_channel(key = key, station = station, **kwargs),[])
    else:
...
def flatten_list(mylist, result = None):
    ...
    for elem in mylist:
        if not isinstance(elem, list):
            if mylist not in result: result.append(mylist)
            break
        else:
...
def select_channel(station, **kwargs):
    ...
    while len(results) >=50:
        ...
        key2 = input('Please enter one or more keywords (separated by blanks) for further specification or "all" to show all entries: ')
        if key2.lower() == "all":
            break
